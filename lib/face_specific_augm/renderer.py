# https://github.com/iacopomasi/face_specific_augm/blob/master/renderer.py
...
def warpImg(img, t_height, t_width, prj, idx):
    ...
    if prj.size != 0:
        if len(np.squeeze( np.asarray( prj[0,:] ) ).astype('float32')) > 32767:  # SHRT_MAX
            pixels1 = cv2.remap(img, np.squeeze( np.asarray( prj[0,:32766] ) ).astype('float32'),\
             np.squeeze( np.asarray( prj[1,:32766]) ).astype('float32'),  cv2.INTER_CUBIC)
            pixels2 = cv2.remap(img, np.squeeze( np.asarray( prj[0, 32766:] ) ).astype('float32'),\
             np.squeeze( np.asarray( prj[1,32766:]) ).astype('float32'),  cv2.INTER_CUBIC)

            pixels = np.vstack((pixels1, pixels2))
        else:
...
def render(img, proj_matrix, ref_U, eyemask, facemask, opts, transformation_mat):
	...
	threedee = np.asmatrix(transformation_mat) * np.vstack((threedee, np.ones((1, threedee.shape[1]))))

	temp_proj = proj_matrix * threedee
